# PEP-7: Upgrading the EDI Identity and Access Management (IAM) Model <a id="top"></a>
...
```
**Listing 1**: Example of an EDI authentication token with fields separated by "*" asterisks. Fields are ordered by user identifier (mark@gmail.com), system namespace (https://pasta.edirepository.org/authentication), time-to-live (1531891534443), and groups (authenticated).
...
**3. Profile linking and unlinking**

Proposition: Instead of physically merging the data from one profile into another, which comes with a list of irreversible consequences, we add a table where we can say that two profiles are linked. When the user then signs in with either of those accounts, we sign them in to their primary profile, and we include all the EDI-IDs from the secondary profile into the EDI token as equivalent principals.So the one token gives the access provided by both profiles.

The main benefit is that the linking becomes fully reversible. So, if we're linking accounts, and we find that the account being linked is already linked to another profile, we don't have to stop in the middle of the procedure, bring up that list of consequences, and give the user an irreversible choice to go ahead and do the merge and delete that second profile. Instead, we can just go through with the linking, and only at the end, after it's done, we can bring up a simpler message saying that we also linked the profile, and if that's not what they wanted, just click the Unlink button. If they click Unlink, they'll be back to exactly where they started. Since linking accounts just means adding a row in the new table of links, no data was modified. After unlinking, they can log into their other account, and it's still there, with the same permissions, etc. That's much cleaner to implement as well, since we don't have to have that extra step inside the Oauth2 flow for the linking (which is a bit tricky to do securely).

The next benefit is that we'll no longer have to maintain the history of merged accounts that clients like ezEML need. As it is now, in the merge procedure I wrote a long time ago (but haven't had time to make the UI for), after merging and deleting the secondary profile, the EDI-ID of the deleted profile is then added to a table that holds that merge history. Since the linked profiles with EDI-IDs are both still in the DB, with the new approach, we can read that information out directly from the linked profiles, so we can remove that table. We can also remove the code that does the merging.

From an implementation point of view, using this link table approach is much safer. If something goes wrong, it's going to be easier to diagnose and undo it than if we had actually merged the two profiles in the DB, and the secondary profile was already deleted.

So for the implementation, we'll be able to remove the merge function, the history table, the extra "Merge Profiles" screen with the "scary" list of irreversible consequences, and the OAuth2 flow associated with that. Instead, we'll add a links table, and logic for including EDI-IDs from linked profiles in the equivalent principals list in the token. The effect is that the one token then gives access provided by both profiles. So it uses our existing system for equivalent principals. Any EDI-IDs added in there just gives the token the access that is provided by those principals. Currently, it holds the public and authenticated access profiles, and the group principals for the memberships.

So, we get safer, user-friendly undo and unlink, we keep all information by not deleting any profiles, and we keep profiles neatly separated by not physically merging them.

As far as I can tell, there are also no issues with linking multiple profiles. E.g., the user has 3 profiles. They link profile 1 with 2, then they sign in to profile 3 and link profile 1, with the end result that all 3 are linked. It also works with the system generated skeleton profiles. It also does not interfere with our current system of linking multiple accounts to a single profile. Linking profiles is just another type of link, separate from  linking accounts to profiles.

...
ACLs for PASTA API methods are declared in a standalone XML document file, `service.xml`, one each for the Data Package Manager (DPM) and Audit Manager (AM) services. These ACLs are read into the system during the PASTA bootstrap process and can be changed by editing the file and restarting the system. ACLs for data package resources are declared in the data package's EML metadata document and are translated into an RDBS table when first uploaded; these ACRs cannot be modified once they are read into the table. Both the `service.xml` file and EML metadata use the same XML `<access>` element structure, allowing use of the same ACR processor to determine authorization.

This PEP proposes the development of an EDI *Authorization service* to replacement the internal authorization processing of the DPM and AM services. This service will improve authorization for applications within the EDI ecosystem in two significant ways: (1)by replacing IdP user identifiers and group identifiers with *PASTA IDs* ([see PEP-2](./pep-2.md)) generated by the EDI *Authentication service* and (2) allowing users to modify ACRs, including adding new ACRs or deleting existing ACRs, at anytime. This second feature significantly alters the current access managemnent model for data packages by allowing data package owners to modify ACRs post-publication.
